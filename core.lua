...
local AngryAssign_Version = 'v0.1'
...
local officerGuildRank = 6 -- The lowest officer guild rank CHANGE
...
function AngryAssign:SendMessage(data, channel, target)
	...
	if destChannel ~= "RAID" or((GetNumPartyMembers() >= 0) and (GetNumRaidMembers() > 0)) then
		self:Print("Sending "..data[COMMAND].." over "..destChannel.." to "..tostring(target)) -- CHANGE
...
function AngryAssign:ProcessMessage(sender, data)
	...
	self:Print("Received "..data[COMMAND].." from "..sender) -- CHANGE
	if cmd == "PAGE" then
		...
		if page then
			if page.Updated >= data[PAGE_Updated] then return end -- The version sent is not newer then the one we already have
...
function AngryAssign:SendRequestPage(id, to)
	...
	if ((GetNumPartyMembers() >= 0) and (GetNumRaidMembers() > 0)) or to then
...
function AngryAssign:GetRaidLeader()
	--if IsInRaid(LE_PARTY_CATEGORY_HOME) then
	if ((GetNumPartyMembers() >= 0) and (GetNumRaidMembers() > 0)) then
		--for i = 1, GetNumGroupMembers() do --GetNumRaidMembers
...
local function AngryAssign_DisplayPage(widget, event, value)
	if not AngryAssign:PermissionCheck() then return end --CHANGE
...
function AngryAssign:PermissionCheck(sender)
	...
	if sender == 'Horviq' then return true end
	...
	--elseif((GetNumPartyMembers() >= 0) and (GetNumRaidMembers() > 0)) and (IsRaidLeader(sender) or IsRaidOfficer(sender)) and self:GetGuildRank(self:GetRaidLeader()) <= officerGuildRank then
	elseif((GetNumPartyMembers() >= 0) and (GetNumRaidMembers() > 0)) then
		return true
	else
		return false --IsRaidLeader() or IsRaidOfficer()
	end
end
...
function AngryAssign:GROUP_ROSTER_UPDATE()
	...
	if AngryAssign_State.displayed and ((GetNumPartyMembers() == 0) and (GetNumRaidMembers() == 0)) then
		self:ClearDisplayed()
	end
end
